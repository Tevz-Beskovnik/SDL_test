...
Object::Object(SDL_Renderer* renderer, int32_t x, int32_t y, std::string path, bool colidable)
    : renderer(renderer), x(x), y(y), path(path), colidable(colidable), objects(*new std::vector<Object *>()), collidable_ids(*new std::vector<uint32_t>()), object_id(-1)
{
...
void Object::register_object(std::vector<Object *>& objects, std::vector<uint32_t>& collidable_ids, uint32_t id) {
    if (default_value) {
        delete &this->objects;
